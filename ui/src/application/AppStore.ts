...
export class AppStore extends BaseStore<IApplication> {
    ...
    @action
    public createmessage = async (title: string, message: string, token: string, priority:number): Promise<void> => {
        await axios.post(`${config.get('url')}message?token=${token}`, {title, message,priority});
        await this.refresh();
        this.snack('Message Has Been Sent');
        console.log(title);
